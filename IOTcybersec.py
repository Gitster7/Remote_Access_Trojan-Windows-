...
class GeneratePayload:
    ...
    def compilate(self):
        ...
        if(self.os == "Linux"):
            ...
            other.printColor("error","[-] IOTcybersec is incompatible with: {}".format(self.os))
            other.printColor("error","[-] please try to restart the tool on Windows or Linux.")
            GeneratePayload.error = True
            exit(1)
...
parser.add_argument("-pa", "--password", dest="PASSWORD", default="OMKARVATSA", help="The password to generate the key to encrypt and decrypt the data. The default password is 'CISCOTHEBOSS'.")
...
try:
    ...
    if(GeneratePayload.error):
        ...
        print("example: /usr/bin/python3 IOTcybersec.py --ip 192.0.0.1" )
